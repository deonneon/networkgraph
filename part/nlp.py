...
character_interactions_str = {
    f"{pair[0]}-{pair[1]}": weight for pair, weight in character_interactions.items()
}

# Save the interactions data to a JSON file for the second script
with open("character_interactions.json", "w") as f:
    json.dump(character_interactions_str, f)
